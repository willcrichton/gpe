...
Point* points_to_cuda(Polygon polygon) {
  ...
  cudaMemcpy(cuda_points, polygon.points, sizeof(Point) * polygon.num_points, cudaMemcpyDeviceToHost);
...
Polygon* polygons_to_cuda(Encoding* img) {
  ...
  cudaMemcpy(cuda_polygons, img->polygons, sizeof(Polygon) * img->num_polygons, cudaMemcpyDeviceToHost);
...
Encoding* encoding_to_cuda(Encoding* img) {
  ...
  cudaMemcpy(cuda_img, img, sizeof(Encoding), cudaMemcpyDeviceToHost);
